...
public class TrainAction extends ActionSupport implements ModelDriven<Train> {
	...
	public String add() throws Exception {
		TrainDao trainDao = new TrainDao();
		System.out.println(train.getEmpno());
		int i=0;
		i=trainDao.add(train);
		if ( i== 0) {
			return ERROR;
		} else {
			i=trainDao.train(train.getEmpno());
			if ( i== 0) {
				return ERROR;
			}
			else{
	...
	public String endtrain() throws Exception {
		TrainDao trainDao = new TrainDao();
		int i=0;
		i=trainDao.endtrain(train);
		if ( i== 0) {
			return ERROR;
		} else {
			i=trainDao.endemp(train.getEmpno());
			if ( i== 0) {
				return ERROR;
			}
			else{
